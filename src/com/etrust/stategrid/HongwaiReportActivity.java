...
import com.etrust.stategrid.adapter.AddHongWaiadapter;
...
//此页面平板进行本地存储标准图
public class HongwaiReportActivity extends Activity implements OnClickListener {

	private ListView task_detial_list;
	private AddHongWaiadapter adapter;
	// take pic
	...
	private void setupView() {
		...
		String[] lable = new String[] { "设备类别","部位","热像特征","故障特征","处理建议", "缺陷照片","I类缺陷标准","II类缺陷标准","III类缺陷标准","备注" };
		String[] data = new String[] {"","","","","","","","","",""};
		adapter = new AddHongWaiadapter(this, lable, data);
	...
	public View.OnClickListener cmtListener=new View.OnClickListener(){
		...
		public void onClick(View arg0) {
			...
			try{
			    if("".equals(ndata[0])){
			    	Toast.makeText(HongwaiReportActivity.this, "请选择设备类别", 1000).show();
			    	return;
			    }
//			    if("".equals(ndata[2])){
//			    	Toast.makeText(AddHongwai.this, "请选择缺陷等级", 1000).show();
//			    	return;
//			    }
//			    if("".equals(ndata[3])){
//			    	Toast.makeText(AddHongwai.this, "请选择缺陷分类", 1000).show();
//			    	return;
//			    }
				...
				//json.put("tsid", tsid);
				json.put("leibie", ndata[0]);
				json.put("buwei", ndata[1]);
				json.put("rexiangtezheng", ndata[2]);
				json.put("guzhangtezheng", ndata[3]);
				json.put("chulijianyi", ndata[4]);
				json.put("picname", pic);
				json.put("Ilei", ndata[6]);
				json.put("IIlei", ndata[7]);
				json.put("IIIlei", ndata[8]);
				json.put("beizhu", ndata[9]);
			...
			if(result>0){
				db.saveUpdateLog("addHongwaibiaozhun", "添加红外标准["+ndata[1]+"]");//数据保存日志
				Toast.makeText(HongwaiReportActivity.this, "保存成功", 2000).show();
				finish();
			}else{
				Toast.makeText(HongwaiReportActivity.this, "保存失败", 2000).show();
				return;
			}			
		}
...
//本地输入数据保存为json
	...
	public int addProDefect(JSONObject json){
		...
		try{
			...
			values.put("leibieid", json.getString("leibie"));
			...
			db.saveSql(Constant.T_hongwai_device, values);
			result=1;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			...
			case activity_result_cropimage_with_data: // 存储器选图
				...
				Cursor cursor = HongwaiReportActivity.this.managedQuery(u,
						proj, null, null, null);
